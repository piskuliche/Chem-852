...
const double global_minimum=-0.5;    // Minimum for histogram
...
void histogram_data(int print, double& value, vector<int>& histogram_storage)
{
    if (print == 0)
    {
        //Calculates a bin number
        int bin_no = (value - global_minimum)/(bin_separation);
        cout << value << " " << bin_no << endl;
...
void metropolis_mc_step(vector<int>& histogram_storage, double& r_old, double& r_new, double& dr, double& U_Old, double& U_New, bool& en_type, int& acc, int& att, double& mc_r, double& mc_r2, double& mc_energy)
{
    ...
    int tmpprnt = 0;
    histogram_data(tmpprnt, tmpsep, histogram_storage);
}
...
int main()
{
    ...
    {
        metropolis_mc_step(histogram_storage, r_old, r_new, dr, U_Old, U_New, en_type, acc, att, mc_r, mc_r2, mc_energy);
        if (i%dumpfreq == 0)
        {
    ...
    int print=1;
    double tmpval=0.0;
    histogram_data(print, tmpval, histogram_storage);
    return 0;
}
